...
class App(ttk.Window):
    def __init__(self):
        super().__init__(themename="litera") # Inicializa com o tema 'litera'
    ...
    def create_main_layout(self):
        ...
        self.sidebar_frame = ttk.Frame(main_container, width=200, bootstyle="light")
    ...
    def create_sidebar(self):
        sidebar_title = ttk.Label(self.sidebar_frame, text="Menu", font=("Arial", 16, "bold"), bootstyle="inverse-light")
        sidebar_title.pack(pady=20, padx=10)

        btn_dashboard = ttk.Button(self.sidebar_frame, text="Dashboard", command=self.show_dashboard, bootstyle="light")
        btn_dashboard.pack(fill=tk.X, pady=5, padx=10)

        btn_settings = ttk.Button(self.sidebar_frame, text="Configurações", state="disabled", bootstyle="light")
        btn_settings.pack(fill=tk.X, pady=5, padx=10)

        btn_about = ttk.Button(self.sidebar_frame, text="Sobre", state="disabled", bootstyle="light")
    ...
    def show_dashboard(self):
        ...
        dashboard_title = ttk.Label(self.main_content_frame, text="Selecione uma Ferramenta de Conversão", font=("Arial", 20, "bold"))
        dashboard_title.pack(pady=20)

        widget_frame = ttk.Frame(self.main_content_frame)
        widget_frame.pack(expand=True)

        btn_webp_to_gif = ttk.Button(widget_frame, text="WebP para GIF", command=lambda: self.show_conversion_screen('webp_to_gif'), bootstyle="primary-outline", padding=20)
        btn_webp_to_gif.grid(row=0, column=0, padx=20, pady=20)

        btn_gif_to_webp = ttk.Button(widget_frame, text="GIF para WebP", command=lambda: self.show_conversion_screen('gif_to_webp'), bootstyle="primary-outline", padding=20)
    ...
    def show_conversion_screen(self, mode):
        ...
        ttk.Label(self.main_content_frame, text=title, font=("Arial", 20, "bold")).pack(pady=10)
        ...
        self.btn_select_files = ttk.Button(files_frame, text="Selecionar Arquivos", command=self.select_files, bootstyle="info")
        self.btn_select_files.pack(side=tk.LEFT, padx=5)
        self.btn_select_folder = ttk.Button(files_frame, text="Selecionar Pasta", command=self.select_folder, bootstyle="info")
        ...
        self.btn_output_dir = ttk.Button(output_frame, text="Escolher Pasta de Destino", command=self.select_output_dir, bootstyle="info")
        ...
        self.btn_preview = ttk.Button(action_frame, text="Pré-visualizar", state="disabled", command=self.open_preview_window, bootstyle="secondary")
        self.btn_preview.pack(side=tk.LEFT, padx=10)
        self.btn_convert = ttk.Button(action_frame, text="Converter", state="disabled", command=self.start_conversion_thread, bootstyle="success")
        self.btn_convert.pack(side=tk.LEFT, padx=10)

        self.progress_bar = ttk.Progressbar(action_frame, orient="horizontal", length=300, mode="determinate", bootstyle="success-striped")
    ...
    def create_webp_to_gif_settings(self, parent):
        ...
        ttk.Checkbutton(parent, text="GIF em Loop (Repetir Infinitamente)", variable=self.loop_var, onvalue=0, offvalue=1, bootstyle="primary-round-toggle").grid(row=1, column=0, columnspan=2, padx=5, pady=5, sticky=tk.W)

    def create_gif_to_webp_settings(self, parent):
        self.lossless_var = tk.BooleanVar()
        ttk.Checkbutton(parent, text="Compressão sem Perdas (Qualidade Máxima)", variable=self.lossless_var, command=self.toggle_quality_slider, bootstyle="primary-round-toggle").grid(row=0, column=0, columnspan=2, sticky=tk.W)
        ...
        self.quality_slider = ttk.Scale(parent, from_=1, to=100, orient=tk.HORIZONTAL, variable=self.quality_var, length=200, bootstyle="info")
    ...
    def start_conversion_thread(self):
        ...
        self.progress_bar.config(bootstyle="success-striped", value=0)
    ...
    def run_conversion(self):
        ...
        for i, file_path in enumerate(self.input_paths):
            if self.conversion_mode == 'webp_to_gif':
                success = convert_webp_to_gif(file_path, self.output_dir, frame_rate=self.fps_var.get(), loop=self.loop_var.get())
            else:
                success = convert_gif_to_webp(file_path, self.output_dir, lossless=self.lossless_var.get(), quality=self.quality_var.get())
